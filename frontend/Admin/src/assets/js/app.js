...
(function ($) {
    ...
    async function obtenerTotalesTransacciones() {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:5000/api/user/transactions-totals', {
                method: 'GET',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Content-Type': 'application/json'
            ...
            if (response.ok) {
                ...
                console.error('Error al obtener los totales de transacciones:', data.message);
            }
        } catch (error) {
            console.error('Error:', error);
        }
    ...
    document.addEventListener('DOMContentLoaded', obtenerTotalesTransacciones);
